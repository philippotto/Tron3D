...
void FenceModel::addFencePart(btVector3 a, btVector3 b)
{
	...
	btScalar angle = (b - a).angle(forward);
	btVector3 axis = (b - a).cross(-forward).normalized();
	
	btDefaultMotionState *fenceMotionState = new btDefaultMotionState(btTransform(btQuaternion(axis, angle), (b + a)/2 + btVector3(0, 0, getFenceHeight() / 2) ));
